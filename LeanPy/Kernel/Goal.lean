...
-/
import LeanPy.Kernel.Elab
import LeanPy.Kernel.Environment
import LeanPy.Kernel.Tactic
...
@[export leanpy_kernel_goal_state_create_m]
...
@[export leanpy_kernel_goal_state_create_from_mvars_m]
...
@[export leanpy_kernel_goal_state_goals]
...
@[export leanpy_kernel_goal_state_meta_context_of_goal]
...
-/
@[export leanpy_kernel_goal_state_resume]
...
-/
@[export leanpy_kernel_goal_state_continue]
...
@[export leanpy_kernel_goal_state_root_expr]
...
@[export leanpy_kernel_goal_state_is_solved]
...
@[export leanpy_kernel_goal_state_get_mvar_e_assignment]
protected def GoalState.getMVarEAssignment (goalState: GoalState) (mvarId: MVarId): Option Expr := do
  ...
  let (expr, _) := instantiateMVarsCore (mctx := goalState.mctx) (e := expr)
  return expr
@[export leanpy_kernel_goal_state_parent_exprs]
...
@[export leanpy_kernel_goal_state_replay_m]
protected def GoalState.replay (dst : GoalState) (src src' : GoalState) : CoreM GoalState :=
  withTraceNode `LeanPy.Kernel.GoalState.replay (fun _ => return m!"replay") do
  ...
  trace[LeanPy.Kernel.GoalState.replay] "Merging ngen {srcNGen.idx} -> ({srcNGen'.idx}, {dstNGen.idx})"
  ...
  trace[LeanPy.Kernel.GoalState.replay] "Merging mvars {src.mctx.mvarCounter} -> ({src'.mctx.mvarCounter}, {dst.mctx.mvarCounter})"
  let mctx := {
    mctx with
  ...
  let m : MetaM _ := Meta.withMCtx mctx do
    ...
    for (lmvarId, l') in src'.mctx.lAssignment do
      ...
      let l' := mapLevel l'
      trace[LeanPy.Kernel.GoalState.replay] "Merging level assignments on {lmvarId.name}"
    ...
    for (mvarId, e') in src'.mctx.eAssignment do
      ...
      let e' ← mapExpr e'
      trace[LeanPy.Kernel.GoalState.replay] "Merging expr assignments on {mvarId.name}"
    ...
    for (mvarId, d') in src'.mctx.dAssignment do
      ...
      trace[LeanPy.Kernel.GoalState.replay] "Merging expr (delayed) assignments on {mvarId.name}"
      unless d == d' do
...
@[export leanpy_kernel_goal_state_try_tactic_m]
...
@[export leanpy_kernel_goal_state_conv_enter_m]
...
@[export leanpy_kernel_goal_state_fragment_exit_m]
...
@[export leanpy_kernel_goal_state_calc_enter_m]
